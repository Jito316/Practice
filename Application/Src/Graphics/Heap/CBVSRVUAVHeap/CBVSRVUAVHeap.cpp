...
const D3D12_GPU_DESCRIPTOR_HANDLE CBVSRVUAVHeap::GetGPUHandle(int _number)
{
	...
	handle.ptr += (UINT64)m_incrementSize * _number;
	return handle;
}

void CBVSRVUAVHeap::SetHeap()
{
	ID3D12DescriptorHeap* ppHeap[] = { m_pHeap.Get() };
	m_pDevice->GetCmdList()->SetDescriptorHeaps(1, ppHeap);
}
