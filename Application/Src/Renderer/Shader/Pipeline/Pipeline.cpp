...
void Pipeline::SetInputLayout(std::vector<D3D12_INPUT_ELEMENT_DESC>& _inputElemnts, const std::vector<InputLayout>& _inputLayouts)
{
	for (int i = 0; i < (int)_inputLayouts.size(); ++i)
	{
		...
		case InputLayout::POSITION:
			...
			break;
		case InputLayout::TEXECOORD:
			_inputElemnts.emplace_back(D3D12_INPUT_ELEMENT_DESC{ "TEXECOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 });
			break;
